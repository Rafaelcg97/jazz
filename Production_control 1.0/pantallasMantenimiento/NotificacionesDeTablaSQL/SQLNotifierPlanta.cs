...
namespace Production_control_1._0.pantallasMantenimiento.NotificacionesDeTablaSQL
{
    class SQLNotifierPlanta
    {
        ...
        public DataTable RegisterDependency()
        {
            ...
            this.CurrentCommand = new SqlCommand("select [id_solicitud], [ubicacion], [modulo], [maquina], [hora_reportada], [hora_asignacion], [hora_apertura], [hora_cierre], [problema_reportado], [corresponde], [prioridad] from dbo.solicitudes where [hora_apertura] is null or [hora_cierre] is null  order by prioridad desc, id_solicitud asc", this.CurrentConnection);
